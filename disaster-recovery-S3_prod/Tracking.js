...
s3Copy.collection.insert(obj,{},function(err){
   ...
   if(err){
    logging.logError(" error in flushing to mongo "+err);
   }else{ logging.logInfo(" successfully flushed to mongo "+new Date()) };
 });
...
noOfObjInGlacier=archiveID.length;
    ...
    s3Copy.find({status:"completed"}).sort({timestamp:-1}).limit(1).exec(function(err, results) { 
      if(err){
        ...
        logging.logError("EROOOOOOOOOOOORRRRRRRR"+err);
      }else{
	if(results==""){
	status="NOT FOUND";
	logging.logInfo("EMPTY RESULTS"+results);
	callback(status);}
	else{
     	s3ObjCnt.find({groupId:results[0].groupId}).sort({timestamp:-1}).limit(1).
  	exec(function(err,result){
	if(err){
		logging.logError("ERRORRR"+err);

}else{if(result==""){
logging.logError("EMPTY RESULT"+result);
status="NOT FOUND";
callback(status);
}else{
    logging.logInfo("RESULT OF ..."+result);
totObjInS3=result[0].totObjects;
logging.logInfo("totObjInS3"+totObjInS3+"noOfObjInGlacier"+noOfObjInGlacier);
      for(var i in result){
         logging.logInfo("RESULT.............."+result);
...
logging.logInfo("90% of total objects in S3"+0.9 * totObjInS3);
...
s3Copy.collection.group(group.key, group.cond, group.initial, group.reduce, group.finalize, 
  true, function(err, results) {
    ...
    logging.logInfo('group results %j', results);
  ...
  copyParams['totObj']=totObj;
   ...
   for(var key in statusArray){
    testArray.push(statusArray[key]);
      logging.logInfo(" Status Array"+JSON.stringify(statusArray)+"KEY"+key+"TESTARRAY"+JSON.stringify(testArray));
...
exports.launchS3Copy=function(req,res){

s3.launchCopyS3();
res.send("S3 copy has been launched");
res.end();
}
